...
export async function PATCH(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  if (v.mode === "ISOLATED") {
    ...
    await db.$transaction(async (tx) => {
      await tx.site.update({ where: { id }, data: { connectorId: v.connectorId, name: v.name, hostname: null, upstreamUrl: v.upstreamUrl, description: v.description, recordSessions: v.recordSessions, clipboardMode: v.clipboardMode, watermark: v.watermark, accessMode: "ISOLATED", ...logoData } });
