...
//
// Multi-tenant (cloud): opens a request transaction on the extended client,
// sets the transaction-local tenant GUC on its connection, and stores the tx in
// ALS so `db` routes every query onto it — Postgres RLS then scopes all access
// to this tenant. Leak-proof: the GUC is transaction-local, so a pooled
// connection never carries it across requests.
//
