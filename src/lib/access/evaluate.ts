...
export type DecisionReason =
  ...
  | "revoked" | "not_yet" | "expired" | "pending_approval";
...
// reason (soonest/most-actionable) → pending_approval > not_yet > expired > revoked.
// (no_grant and user_disabled are handled before this table, never compared here.)
const DENY_PRIORITY: Record<"pending_approval" | "not_yet" | "expired" | "revoked", number> = {
  pending_approval: 4, not_yet: 3, expired: 2, revoked: 1,
};
...
export async function evaluateAccess(userId: string, siteId: string, now: Date): Promise<Decision> {
  ...
  let best: "pending_approval" | "not_yet" | "expired" | "revoked" | null = null;
  for (const g of grants) {
    const d = classifyGrant(g, now);
    if (d === "allow") return { allow: true, reason: "allow" };
    const dd = d as "pending_approval" | "not_yet" | "expired" | "revoked";
